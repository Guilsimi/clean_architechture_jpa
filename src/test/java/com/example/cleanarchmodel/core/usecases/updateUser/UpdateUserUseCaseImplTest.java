...
class UpdateUserUseCaseImplTest {
    ...
    void updateUserSuccessfully() {
        ...
        UserUpdateData userNewData = new UserUpdateData("UpdateTest", "UpdateUser", null);
        ...
        when(this.userRepository.update(userObject)).then(invocation -> {
            userObject.updateData(userNewData);
            return null;
        });

        this.updateUserUseCase.execute(userId, userNewData);
    ...
    void throwExceptionWhenUserNotFound() {
        ...
        RuntimeException exception = assertThrows(UserNotFoundException.class, () -> {
            this.updateUserUseCase.execute(UUID.randomUUID(), new UserUpdateData("", "", ""));
        });
